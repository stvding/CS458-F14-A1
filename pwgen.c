...
//Checks if the tmp file exists, and if it has the correct permissions
...
//TODO: Fix this, this is a terrible hack
static
void convert_uc_c(unsigned char * buf, char * res, size_t size) {
  int i;
  unsigned char c;
  for (i = 0; i < size; i++) {
    c = (buf[i] >> 2);
...
//Generate a password
static
void gen_passwd(pwgen_args args, char * passwd) {
...
static
char * gen_crypt(pwgen_args args, char * password) {
...
static
pwgen_args parse_args(int argc, char* argv[]) {
  int c, res;
  pwgen_args args;
